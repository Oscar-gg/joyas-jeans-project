...
import { FitEdit } from "../edit/FitEdit";

export const FitTable = ({ search }: { search?: string }) => {
  const { data: colors, isLoading } = api.get.getFitIds.useQuery({
    search: search,
  });
...
const headers = ["Nombre", "Descripción", "Productos asociados", "Modificar"];

const TableRow = ({ itemId }: { itemId: string }) => {
  const { data: rowData, isLoading } = api.get.getFitById.useQuery({
    id: itemId,
  });
  ...
  return (
    <tr className="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
      {seeModal && (
        <EditModal title={"Editar Fit"} setSeeModal={setSeeModal}>
          <FitEdit fit={rowData} />
        </EditModal>
      )}
      <th
