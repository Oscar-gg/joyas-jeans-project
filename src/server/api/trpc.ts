...
interface Meta {
  role: "unauthenticated" | "authenticated" | "organizationMember" | "admin";
}
...
const enforceUserIsAuthed = t.middleware(({ ctx, next, meta }) => {
  if (!ctx.session?.user) {
  ...
  if (meta?.role) {
    if (
      ...
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: `Expected: ${meta.role}, current: ${ctx.session.user.role}`,
      });
    }
  }
